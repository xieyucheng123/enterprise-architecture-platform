...
/// Adds a nullable `description` column to the `organizations` table.
///
/// The column stores an optional human-readable description of an
/// organization. It is nullable with a default of `NULL` so existing
/// rows are backfilled without requiring a value.
///
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(
            r#"ALTER TABLE "organizations" ADD COLUMN "description" TEXT"#,
        )
        .await?;
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(r#"ALTER TABLE "organizations" DROP COLUMN "description""#)
            .await?;
        Ok(())
    }
}